...
from Model import DeepLabV3PlusModel, DeepLabV3PlusModel2, DeepLabV3PlusModel3B, DeepLabV3PlusModel3RP, DeepLabV3PlusModel3RWithFreezeAndDropout
from SegmentationDataset import ImageWithMaskPatchesDataset as SegmentationDataset
...
def custom_collate_fn(batch):
    ...
    for item in batch:
        all_patches.append(item['patches'])  # shape: (n_patches, C, H, W)
        all_coords.extend(item['coords'])
        all_image_ids.extend([item['image_id']] * len(item['patches']))
        masks.append(item['mask'])

    patches_tensor = torch.cat(all_patches, dim=0)
    masks_tensor = torch.stack(masks)  # shape: (batch_size, H, W)
    return {
        'patches': patches_tensor,         # shape: (total_patches, C, H, W)
        'coords': all_coords,              # list of (y, x)
        'image_ids': all_image_ids,        # list of idx, one per patch
        'masks': masks_tensor              # shape: (batch_size, H, W)
    }
...
if __name__== "__main__":
    ...
    train_set = SegmentationDataset(
        image_paths=sorted(glob.glob(f"{x_train_dir}/*.jpg")),  
        mask_paths=sorted(glob.glob(f"{y_train_dir}/*.png")),
        patch_size = 256,
        overlap=128,
    ...
    valid_set= SegmentationDataset(
        image_paths=sorted(glob.glob(f"{x_valid_dir}/*.jpg")),  
        mask_paths=sorted(glob.glob(f"{y_valid_dir}/*.png")),
        patch_size = 256,
        overlap=128,
    )

    test_set= SegmentationDataset(
        image_paths=sorted(glob.glob(f"{x_test_dir}/*.jpg")),  
        mask_paths=sorted(glob.glob(f"{y_test_dir}/*.png")),
        patch_size = 256,
        overlap=128,
        )
    ...
    bs, nw = 2, 4
    train_loader = DataLoader(train_set, batch_size=bs, shuffle=True, num_workers=nw, collate_fn=custom_collate_fn)
    val_loader = DataLoader(valid_set, batch_size=bs, shuffle=False, num_workers=nw, collate_fn=custom_collate_fn)
    test_loader = DataLoader(test_set, batch_size=bs, shuffle=False, num_workers=nw, collate_fn=custom_collate_fn)
    ...
    model = DeepLabV3PlusModel3RP( encoder_name=encoder_name, 
